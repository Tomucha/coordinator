...
public class OrganizationApiController extends AbstractApiController {
    ...
    public @ResponseBody ApiResponse organizationEvents(HttpServletRequest request) {
        UserRequest<OrganizationEventsRequestParams> req = parseRequest(request, OrganizationEventsRequestParams.class);

        OrganizationInEventFilter filter = new OrganizationInEventFilter();
        filter.setOrganizationIdVal(req.params.getOrganizationId() != null ? req.params.getOrganizationId() : req.user.getOrganizationId());
